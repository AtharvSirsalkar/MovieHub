...
import { asynchloadtv, removetv } from "../../store/actions/tvActions";
import {
  Link,
  Outlet,
  useLocation,
...
const Tvdetails = () => {
  ...
  return info ? (
    <div className="w-screen relative overflow-x-hidden ">
      <img
        style={{ opacity: ".5" }}
        className="object-cover bg-cover w-full h-[210vh] opacity-50 blur-sm bg-gradient-to-b "
      ...
      <div className=" flex flex-col gap-y-5 absolute z-10 top-[72%] h-[10vh] px-20">
        {info.watchproviders && info.watchproviders.flatrate ? (
          <div className="font-semibold flex gap-x-5 text-white">
            ...
            {info.watchproviders.flatrate.map((flatrate, i) => (
              <img
                key={i}
      ...
      <div className="absolute top-[93%] w-[90vw] font-bold ml-20 text-3xl">
        <hr className="h-[2px] border-none bg-zinc-400 mb-2" />
        <h1 className=" text-white">Seasons</h1>
      </div>
      <div className="absolute top-[102%] w-full flex items-center overflow-x-auto justify-center">
        <div className="w-[90vw] h-[55vh]   flex duration-200 ease-out gap-5  overflow-y-hidden">
          {info.details.seasons.length > 0 ? (
            info.details.seasons.map((elem, i) => (
              <div className=" flex flex-col gap-2 items-start justify-center shrink-0 max-w-[15vw]">
                <img
                  
                  className=" w-full rounded-t-lg duration-500 img-transition ease-out object-contain"
      ...
      {/* part 5 recommondations and similarities */}
      <div className="absolute top-[158%] w-[90vw] font-bold ml-20 text-3xl">
        <hr className="h-[2px] border-none bg-zinc-400 mb-2" />
        <h1 className=" text-white">Recommendations & Similar stuff</h1>
      </div>

      <div className="absolute top-[163%] w-[100vw] flex items-center justify-center">
